...
main = do
    ...
    -- BEGIN: quick 'n' dirty SVG writer
    ...
    let rainbox@[red,green,blue,yellow,purple,teal] = 
            map colorize [(255,0,0)
    ...
    -- now to read in an svg of the park
    let readPoint :: String -> Point
        readPoint s | Just [x,y] <- matchRegex (
            mkRegex "([0-9.]+),([0-9.]+)") s = (read x,read y)
    ...
    -- now we need to slice things up to assign seats!
